...
<hr />
<h2 class="eint-heading-icon eint-icon-add">Start New Translation for the {{lnginfo.name}} language</h2>
<form class="form-inline well" action="/makelanguage/{{pmd.name}}/{{lnginfo.isocode}}" method="post" enctype="multipart/form-data">
    <fieldset>
        <br />
        You are about to add the {{lnginfo.name}} language as translation language to the {{pmd.human_name}} project. Are you sure?
        <br />
