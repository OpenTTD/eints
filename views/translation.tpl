%rebase('main_template', title='language ' + language + ' in ' + pmd.human_name)
<h1>
...
<a class="btn pull-right" href="/download/{{pmd.name}}/{{language}}"><i class="icon-download"></i> Download Lang File</a>
<div class="btn-group">
    ...
    % if len(strs) > 0:
        % for sdd in strs:
            ...
            % if is_blng:
                <dt>{{sdd.sname}}</dt>
            % else:
                <li><a href="/string/{{pmd.name}}/{{language}}/{{sdd.sname}}">{{sdd.sname}}</a></li>
            % end
